...
const EPGModal: React.FC<EPGModalProps> = ({
  ...
  const [guideData, setGuideData] = useState<EPGChannel[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [categoryGroups, setCategoryGroups] = useState<Record<string, Channel[]>>({});
  const [timeSlots, setTimeSlots] = useState<Date[]>([]);

  // Group channels by category on mount
  useEffect(() => {
    if (channels && channels.length > 0) {
      const groups: Record<string, Channel[]> = { "All Channels": channels };
      ...
      // Simulate loading EPG data (in a real app, this would fetch from the API)
      setTimeout(() => {
        setIsLoading(false);
      }, 1000);
    }
      ...
      <DialogContent className="max-w-4xl h-[80vh] flex flex-col p-0">
        ...
        {isLoading ? (
          <div className="flex items-center justify-center flex-1">
            ...
            <span className="ml-2">Loading program guide...</span>
          </div>
        ) : (
          <div className="flex-1 flex flex-col overflow-hidden">
            <Tabs defaultValue="All Channels" className="flex-1 flex flex-col">
              ...
              {Object.keys(categoryGroups).map(group => (
                ...
                >
                  <ScrollArea className="flex-1">
                    {categoryGroups[group].map(channel => (
                      <div 
                        key={channel.id} 
