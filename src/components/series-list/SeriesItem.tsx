...
const SeriesItem: React.FC<SeriesItemProps> = ({
  ...
  return (
    <motion.div
      ...
      className={`relative rounded-xl overflow-hidden h-full flex flex-col bg-gradient-to-b from-black/30 to-black/10 backdrop-blur-sm 
    ...
    >
      ...
      <div className="p-3 flex-1 flex flex-col">
        <h3 className="font-medium text-white group-hover:text-primary transition-colors duration-300 line-clamp-1">{series.name}</h3>
        <div className="flex flex-wrap gap-2 mt-2 text-xs">
          {series.year && (
            <div className="bg-white/10 text-white/70 px-2 py-1 rounded-md flex items-center">
              <CalendarIcon className="h-3 w-3 mr-1 text-white/50" />
              {series.year}
            </div>
          )}
          {series.rating && (
