...
const PlaylistInput: React.FC<PlaylistInputProps> = ({ onPlaylistLoaded, className }) => {
  ...
  const validateUrl = (url: string): boolean => {
    ...
    if (!validateAllowedUrl(url)) {
      setUrlError("Only URLs from amri.wtf, deliverynetwork.online, or xerotv.vip are allowed");
      return false;
    }
  ...
  const validateXtreamServer = (server: string): boolean => {
    ...
    if (!validateAllowedUrl(server)) {
      setServerError("Only servers from amri.wtf, deliverynetwork.online, or xerotv.vip are allowed");
      return false;
    }
  ...
  const loadSamplePlaylist = async (url: string, name: string) => {
    if (!validateAllowedUrl(url)) {
      toast.error("Only URLs from amri.wtf, deliverynetwork.online, or xerotv.vip are allowed");
      return;
    }
      ...
      <Tabs defaultValue="file" className="w-full">
        ...
        <TabsContent value="file" className="space-y-4">
          ...
          <div className="text-xs text-amber-500 mt-2 mb-4">
            Note: Only playlists with URLs from amri.wtf, deliverynetwork.online, or xerotv.vip will be allowed to load.
          </div>
          ...
          <form onSubmit={handleUrlSubmit} className="space-y-4">
            <div>
              ...
              <p className="text-xs text-amber-500 mt-2">
                Only URLs from amri.wtf, deliverynetwork.online, or xerotv.vip are allowed.
              </p>
            </div>
          </form>
        </TabsContent>
          ...
          <form onSubmit={handleXtreamSubmit} className="space-y-4">
            <div>
              ...
              <p className="mt-3 text-xs text-amber-500">
                Only servers from amri.wtf, deliverynetwork.online, or xerotv.vip are allowed.
              </p>
